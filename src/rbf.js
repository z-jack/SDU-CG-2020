...
export default function RBFFactory(buffer) {
  ...
  const pointsXY = points.map((point) => point.slice(0, 2));
  const pointsZ = points.map((point) => point[2]);
  const rbfi = RBF(pointsXY, pointsZ);
  const gridMinMax = pointsXY.reduce(
    (p, v) => [
      Math.min(p[0], v[0] + 0.01),
      Math.max(p[1], v[0] - 0.01),
      Math.min(p[2], v[1] + 0.01),
      Math.max(p[3], v[1] - 0.01),
    ],
    [Infinity, -Infinity, Infinity, -Infinity]
  );
  const valueMinMax = pointsZ.reduce(
    (p, v) => [Math.min(p[0], v - 0.01), Math.max(p[1], v + 0.01)],
    [Infinity, -Infinity]
  );
  const grid = Array(100)
    ...
    .map((_, row) =>
      Array(100)
        ...
        .map((_, col) => {
          const x =
            ((gridMinMax[1] - gridMinMax[0]) / 99) * row + gridMinMax[0];
          const y =
            ((gridMinMax[3] - gridMinMax[2]) / 99) * col + gridMinMax[2];
          const [z] = rbfi([x, y]);
          if (z < valueMinMax[0] || z > valueMinMax[1]) return undefined;
          return [x, y, z];
        })
        .filter((p) => p)
    )
...
function RBF(points, values, distanceFunction, epsilon) {
  ...
  if (distanceFunction) {
    distance =
      typeof distanceFunction !== "string"
        ? distanceFunction
  ...
  var sample = values[0];
  var D = typeof sample === "number" ? 1 : sample.length;
